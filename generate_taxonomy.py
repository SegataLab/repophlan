...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.error("Error in downloading, extracting, or reading " +
                     par['ncbi_taxdump'] + ": " + str(e))
        sys.exit()
    ...
    if par['output']:
        logger.info(
            'Exporting the original unedited NCBI taxonomy to: ' + par['output'])
        tax_tree.print_tree(par['output'])
        logger.info(par['output'] + ' saved.')

    logger.info(
        'Processing the taxonomy for consistency and a fixed number of taxonomic levels')
    tax_tree.get_tree_with_reduced_taxonomy(logger=logger)
    logger.info('Finished postprocessing the taxonomy')

    if par['output_red']:
        logger.info('Exporting the edited NCBI taxonomy to: ' +
                    par['output_red'])
