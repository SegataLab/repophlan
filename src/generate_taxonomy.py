...
class Accessions:
    ...
    def __init__( self, prokaryotes, eukaryotes, viruses, scaffs_in_complete, catalog ):
        self.accessions = {}
	...
	print "doing",prokaryotes
        ...
        with open( eukaryotes ) as inpf:
			...
			print line[18]
                ...
                if not gen_seqs:
                    if taxid not in NW_dict:
                        continue
                    else:
                        self.accessions[taxid] = { 'name' : NW_dict[taxid]["name"],
                                           'status' : status,
                                           'gen_seqs' : NW_dict[taxid]["WGS"] }
		else:
                	self.accessions[taxid] = { 'name' : name,
                                           'status' : status,
                                           'gen_seqs' : gen_seqs }
