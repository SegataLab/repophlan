...
if __name__ == '__main__':
    ...
    for seq_record in SeqIO.parse(f, "genbank"):
        ...
        if bioproject is None:
            logger.warning('Accession ' + accession +
                           ' without Bioproject, skipping it')
            continue

        if bioproject not in viruses:
            viruses[bioproject] = {'ffn': [],
                                   'faa': [], 'fna': [], 'taxon': ""}
        ...
        nrec += 1
        logger.info('Processing ' + accession +
                    ' [record # ' + str(nrec) + '] ')
    ...
    for nrec, (k, v) in enumerate(viruses.items()):
        ...
        logger.info('Files exported for ' + k + ' [record # ' + str(nrec) + '] ' + ",".join(
            [outdir + k + ".fna", outdir + k + ".ffn", outdir + k + ".faa"]))
